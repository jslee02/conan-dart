...
class BoostConan(ConanFile):
    ...
    def source(self):
        src_name = "%s.zip" % self.FOLDER_NAME if sys.platform == "win32" else "%s.tar.gz" % self.FOLDER_NAME
        url = "http://sourceforge.net/projects/boost/files/boost/%s/%s/download" % (self.version, src_name)
        self.output.info("Downloading %s..." % url)
        tools.download(url, src_name)
        if sys.platform == "win32":
            tools.unzip(src_name, ".")
        else:
            tools.untargz(src_name, ".")
        os.unlink(src_name)
